...
from app.llms import get_local_llm
...
class ConfigurableAgent(RunnableBinding):
    ...
    def __init__(
        ...
        *,
        tools: Sequence[Tool],
        agent: AgentType = AgentType.QWEN,
        system_message: str = DEFAULT_SYSTEM_MESSAGE,
        assistant_id: Optional[str] = None,
        thread_id: Optional[str] = "",
        retrieval_description: str = RETRIEVAL_DESCRIPTION,
...
chatbot = (
    ConfigurableChatBot(llm=LLMType.QWEN, checkpoint=CHECKPOINTER)
    .configurable_fields(
    ...
    def __init__(
        ...
        **others: Any,
        ...
        if llm_type == LLMType.QWEN:
            ...
            raise ValueError("Unexpected llm type")
        chatbot = get_dual_retrieval_executor(
            llm, gov_retriever, company_retriever, system_message, CHECKPOINTER
        )
        super().__init__(
            llm_type=llm_type,
            system_message=system_message,
...
chat_retrieval = (
    ConfigurableRetrieval(llm_type=LLMType.QWEN, checkpoint=CHECKPOINTER)
    .configurable_fields(
