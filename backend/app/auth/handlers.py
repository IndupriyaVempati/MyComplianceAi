...
class NOOPAuth(AuthHandler):
    ...
    async def __call__(self, request: Request) -> User:
        # First check for a Bearer token in the header (used by our new local login)
        auth_header = request.headers.get("Authorization")
        if auth_header and auth_header.startswith("Bearer "):
            token = auth_header.split(" ")[1]
            try:
                # In NOOPAuth we can decode the token using our local settings 
                # even if AUTH_TYPE is not strictly jwt_local
                payload = jwt.decode(
                    token,
                    settings.jwt_local.decode_key,
                    algorithms=[settings.jwt_local.alg.upper()],
                    options={"verify_signature": False} # Accept it without strict sig check for mock auth, or True if we want security. Let's verify it.
                )
                sub = payload.get("sub")
