...
function DocumentViewer(props: {
  ...
  if (!open) {
    return (
      <button
        className={cn(
          "flex items-start min-w-0 max-w-full gap-4 overflow-hidden px-4 transition-colors hover:bg-gray-50/50 active:bg-gray-50 dark:hover:bg-white/5 dark:active:bg-white/10",
          props.className,
        )}
        onClick={() => setOpen(true)}
      >
        <ChevronRightIcon className="mt-[6px] h-4 w-4 text-gray-500 dark:text-[#8e8ea0]" />
        <StringViewer
  ...
  return (
    <div
      className={cn(
        "flex items-start gap-4 px-4 text-left transition-colors hover:bg-gray-50/50 active:bg-gray-50 dark:hover:bg-white/5 dark:active:bg-white/10",
        props.className,
      )}
    >
      <button onClick={() => setOpen(false)}>
        <ChevronDownIcon className="mt-[6px] h-4 w-4 text-gray-500 dark:text-[#8e8ea0]" />
      </button>
      ...
      <span className="flex flex-grow basis-0 flex-col gap-4">
        ...
        <span className="flex flex-col flex-wrap items-start gap-2">
          {metadata.map(({ key, value }, idx) => {
            return (
              <span
                className="rounded-md bg-gray-100 p-1 px-1.5 text-xs dark:bg-[#323232]"
                key={idx}
              >
...
export function DocumentList(props: {
  ...
  return (
    <div className="flex flex-col items-stretch gap-4 rounded-lg ring-1 ring-gray-300 dark:ring-[#3a3a3a] overflow-hidden my-2">
